...
<?php // Inicia um bloco de código PHP
		...
		// Verifica se o formulário foi enviado e o botão "Cadastrar" foi pressionado.	
			$conn = new caminho();
            $query = "INSERT INTO tbProduto (nome_prod, valor_prod, desc_prod, foto, qtde) VALUES (:nome, :preco, :descricao, :nome_imagem, :quantidade)";
            $foto = $_FILES["imagem"]; 
			...
			// Verifica se um arquivo foi enviado, e se o campo nome não está vazio através da condição "empty".	
				...
				// Verifica se não houve erros durante a validação da imagem.
                    ...
                    $cadastrar->bindParam(':preco',$_POST['cxpreco'],PDO::PARAM_INT);
                    $cadastrar->bindParam(':descricao',$_POST['cxdesc'],PDO::PARAM_STR);
                    $cadastrar->bindParam(':nome_imagem',$nome_imagem,PDO::PARAM_STR);
                    $cadastrar->bindParam(':quantidade',$_POST['cxqtd'],PDO::PARAM_INT);
                    ...
                    {
                        echo ('<script>Produto Não Cadastrado, tente novemente.</script>');
                    } 
			...
			// Se não houve erros durante a validação da imagem.
				...
				echo('<script>window.alert("Você não selecionou nenhuma arquivo!");window.location="../../view/vendedor/cad_produto.php";</script>'); // Exibe um alerta JavaScript informando ao usuário que nenhum arquivo foi selecionado e redireciona-o de volta para a página "cadastro.php".
			}
		}
